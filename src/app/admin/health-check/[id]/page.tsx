...
export default async function AdminHealthCheckPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const { supabase } = await requireStaff();
  const { data: submission } = await supabase.from("health_check_submissions").select("id, user_id, property_id, score, result_level, status, submitted_at").eq("id", id).single();
