...
export default async function SuburbReportAdminPage() {
  ...
  const { data: reports } = await supabase.from("suburb_reports").select("id, name, postcode, zone, published, report_date, storage_path, updated_at").order("name");
  return <main className="min-h-screen bg-mist px-5 py-10 sm:px-8 lg:px-12"><div className="mx-auto max-w-6xl">
    ...
    <header className="mt-8 border-b border-line pb-8"><p className="text-sm font-semibold uppercase tracking-[0.18em] text-coastal">Administrator only</p><h1 className="mt-3 font-display text-4xl font-medium text-navy sm:text-5xl">Suburb Report CMS</h1><p className="mt-3 max-w-2xl text-muted">Upload a new Gold Coast suburb report or replace an existing suburb&apos;s PDF.</p></header>
    <section className="mt-8 rounded-2xl border border-line bg-white p-6 shadow-card sm:p-8"><h2 className="text-xl font-semibold text-navy">Upload report</h2><SuburbReportForm /></section>
    <section className="mt-8 rounded-2xl border border-line bg-white p-6 shadow-card sm:p-8"><h2 className="text-xl font-semibold text-navy">Report library</h2><div className="mt-5 divide-y divide-line">{(reports ?? []).map((report) => <article key={report.id} className="grid gap-3 py-5 sm:grid-cols-[1fr_auto] sm:items-center"><div><h3 className="font-semibold text-navy">{report.name} <span className="font-normal text-muted">{report.postcode}</span></h3><p className="mt-1 text-sm text-muted">{report.zone} · {report.report_date || "No report date"}</p></div><span className={`inline-flex items-center gap-2 justify-self-start rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-wider ${report.published ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-muted"}`}><FileText className="size-3.5" />{report.published ? "Published" : report.storage_path ? "Draft" : "Awaiting PDF"}</span></article>)}</div></section>
  </div></main>;
}
