...
import { ClipboardCheck, Clock3, Send, ArrowLeft, ArrowRight, UsersRound, FileUp } from "lucide-react";
...
export default async function AdminPage() {
  ...
  return <main className="min-h-screen bg-mist px-5 py-10 sm:px-8 lg:px-12"><div className="mx-auto max-w-7xl">
    ...
    <header className="mt-8 flex flex-wrap items-end justify-between gap-6 border-b border-line pb-8"><div><p className="text-sm font-semibold uppercase tracking-[0.18em] text-coastal">{role.replace("_", " ")}</p><h1 className="mt-3 font-display text-4xl font-medium text-navy sm:text-5xl">Admin Dashboard</h1><p className="mt-3 text-muted">Assessment activity requiring your team&apos;s attention.</p></div>{role === "admin" && <div className="flex flex-wrap gap-3"><Link href="/admin/suburb-reports" className="inline-flex h-11 items-center gap-2 rounded-xl border border-line bg-white px-5 text-sm font-semibold text-navy"><FileUp className="size-4" />Suburb reports</Link><Link href="/admin/users" className="inline-flex h-11 items-center gap-2 rounded-xl bg-navy px-5 text-sm font-semibold text-white"><UsersRound className="size-4" />Manage roles</Link></div>}</header>
