...
export default async function BlogArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data: post } = await supabase.from("blog_posts").select("title, excerpt, body, cover_image_path, video_url, author_name, published_at, created_at").eq("slug", slug).eq("published", true).single();
  if (!post) notFound();
  ...
  return <PageContainer><article className="mx-auto max-w-4xl">
    ...
    {imageUrl && <div className="relative mt-8 aspect-[16/9] overflow-hidden rounded-2xl bg-mist"><Image src={imageUrl} alt="" fill sizes="(min-width: 1024px) 896px, 100vw" className="object-cover" /></div>}
    <div className="mt-9 space-y-6 text-base leading-8 text-ink">{paragraphs.map((paragraph, index) => <p key={index} className="whitespace-pre-line">{paragraph}</p>)}</div>
    {video && <section className="mt-10"><h2 className="font-display text-3xl font-medium text-navy">Watch the video</h2><div className="mt-5 aspect-video overflow-hidden rounded-2xl bg-black">{video.type === "iframe" ? <iframe src={video.src} title={`Video: ${post.title}`} className="size-full" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen /> : <video src={video.src} controls className="size-full" />}</div></section>}
