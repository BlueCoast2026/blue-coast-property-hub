...
export default async function ReadyToRentResultPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: submission } = await supabase.from("ready_to_rent_submissions").select("id, property_id, score, result_level, status").eq("id", id).eq("user_id", user.id).single();
  ...
  const [{ data: property }, { data: feedback }, { data: attachments }] = await Promise.all([
    ...
    supabase.from("admin_notes").select("id, note, created_at").eq("submission_type", "ready_to_rent").eq("submission_id", id).order("created_at", { ascending: false }),
    supabase.from("review_attachments").select("id, file_name, created_at").eq("submission_type", "ready_to_rent").eq("submission_id", id).order("created_at", { ascending: false }),
  ]);
  ...
  return <PageContainer><div className="mx-auto max-w-3xl"><section className="overflow-hidden rounded-2xl border border-line bg-white shadow-card">
    ...
    <div className="p-7 sm:p-10"><CheckCircle2 className="size-9 text-coastal" /><h1 className="mt-5 font-display text-4xl font-medium text-navy">{content.heading}</h1><p className="mt-4 text-lg leading-8 text-muted">{content.description}</p>
      ...
      {feedback?.length ? <section className="mt-8"><h2 className="text-xl font-semibold text-navy">Feedback from Blue Coast Realty</h2><div className="mt-4 space-y-3">{feedback.map((item) => <article key={item.id} className="rounded-xl border border-line p-5"><p className="leading-7 text-ink">{item.note}</p><p className="mt-3 text-xs text-muted">{new Intl.DateTimeFormat("en-AU", { dateStyle: "medium", timeStyle: "short" }).format(new Date(item.created_at))}</p></article>)}</div></section> : null}
      {attachments?.length ? <section className="mt-8"><h2 className="text-xl font-semibold text-navy">Documents from Blue Coast Realty</h2><div className="mt-4 space-y-2">{attachments.map((item) => <a key={item.id} href={`/api/review-attachments/${item.id}`} className="flex items-center gap-3 rounded-xl border border-line p-4 font-medium text-navy hover:bg-mist"><FileText className="size-5 text-coastal" />{item.file_name}<Download className="ml-auto size-4" /></a>)}</div></section> : null}
      <Link href="/dashboard" className="mt-8 inline-flex h-11 items-center gap-2 rounded-xl bg-navy px-5 text-sm font-semibold text-white"><ArrowLeft className="size-4" />Back to Property Hub</Link>
    </div>
