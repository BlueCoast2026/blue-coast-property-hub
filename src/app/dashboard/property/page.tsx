...
import { ClipboardCheck, Home, KeyRound, MapPin, Scale } from "lucide-react";
...
export default async function PropertyPage() {
  ...
  return <PageContainer>
    ...
    {(properties ?? []).length > 0 && <section className="mb-8 grid gap-5 lg:grid-cols-2" aria-label="Your properties">{properties!.map((property) => <article key={property.id} className="rounded-2xl border border-line bg-white p-6 shadow-card"><MapPin className="size-5 text-coastal" /><h2 className="mt-5 text-xl font-semibold text-navy">{property.address_line_1}</h2><p className="mt-2 text-muted">{[property.suburb, property.state, property.postcode].filter(Boolean).join(" ")}</p><div className="mt-6 flex flex-wrap gap-3">
      <Link href={`/dashboard/health-check?property=${property.id}`} className="inline-flex h-11 items-center gap-2 rounded-xl bg-navy px-4 text-sm font-semibold text-white"><ClipboardCheck className="size-4" />Health Check</Link>
      <Link href={`/dashboard/ready-to-rent?property=${property.id}`} className="inline-flex h-11 items-center gap-2 rounded-xl border border-line px-4 text-sm font-semibold text-navy"><KeyRound className="size-4" />Ready to Rent</Link>
      <Link href={`/dashboard/property-decision?property=${property.id}`} className="inline-flex h-11 items-center gap-2 rounded-xl border border-line px-4 text-sm font-semibold text-navy"><Scale className="size-4" />Decision Check</Link>
      <DeletePropertyForm propertyId={property.id} address={property.address_line_1} />
    </div></article>)}</section>}
