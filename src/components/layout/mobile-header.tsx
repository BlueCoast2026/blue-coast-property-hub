...
export function MobileHeader({ language }: { language: AppLanguage }) {
  ...
  return <header className="sticky top-0 z-20 border-b border-line bg-white/95 px-5 py-3 backdrop-blur lg:hidden">
    <div className="flex items-center justify-between"><RealtyLogo size="small" /><div className="flex items-center gap-2"><LanguageSwitcher language={language} /><button onClick={() => setOpen((value) => !value)} className="grid size-11 place-items-center rounded-xl border border-line text-navy" aria-label="Toggle navigation" aria-expanded={open}>{open ? <X className="size-5" /> : <Menu className="size-5" />}</button></div></div>
